...
def product_details(request,cname,pname):
    if(Catagory.objects.filter(name=cname,status=0)):
        if(Product.objects.filter(name=pname,status=0)):
            ...
            messages.error(request,'No such product mot founded')
            return redirect('collections')
    else:
        messages.error(request,'No such catagory not found')
        return redirect('collections')
...
def add_to_cart(request):
    if request.headers.get('x-requested-with') == 'XMLHttpRequest':
        ...
        return JsonResponse({'status':'Invalid accesss'},status=200)
        ...
        if request.user.is_authenticated:
            ...
            if product_status:
                 if Favourite.objects.filter(user=request.user.id,product_id=product_id):
                    ...
                    return JsonResponse({'status':'Product Add To Favourate'}, status=200)
          

        else:
            return JsonResponse({'status':'Login To Add Favourate'}, status=200)
    else:
        return JsonResponse({'status':'Invalid accesss'},status=200)
...
def place_order(request):
    ...
    if request.user.is_authenticated:
        if request.method == 'POST':
            ...
            trackno='akhil'+str(random.randint(1111111,9999999))
...
def searchproduct(request):
    if request.method == 'POST':
        ...
        if searchedterm == " ":
            ...
            if product:
                return redirect("product_details/"+ product.catagory.name+'/'+product.name)
            else:
